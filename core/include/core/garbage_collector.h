...
/**
 * Garbage collection of the dedup system.
 *
 * The deduplication garbage collection works in the following steps:
 * - If a block is written and all containers of it are committed, a BlockMappingWritten with a commit flag is
 *   written. If a block mapping is written, but not all container of the block chunks are already committed, a
 *   BlockMappingComitted event is committed at a later point. These messages contain the original block mapping
 *   and the new block mapping.
 *
 * - The gc processes these events during a background log processing and calculates a diff. The diff contains
 *   informations about the chunks used more in the new mapping (usage count increase) or are not used or used less
 *   in the new block mapping (usage count decrease). These update usage counts are written to the chunk index.
 *
 * -
 */
class GarbageCollector: public dedupv1::log::LogConsumer,
        public dedupv1::IdleTickConsumer,
        public dedupv1::StatisticProvider {
    private:
        friend class GarbageCollectorTest;
        friend class ProcessMappingDiffTask;
        friend class ProcessMappingDirtyDiffTask;
        friend class ProcessMappingDiffParentTask;
        FRIEND_TEST(GarbageCollectorTest, TriggerByStartProcessing);
        FRIEND_TEST(GarbageCollectorTest, TriggerByIdleStart);
         ...
         * The key is a combination of the container id and the fingerprint of the candidate. The size of therefore
         * fingerprint size + 8 bytes. the container id is stored in the higher-value bits. This is done to
         * process all fingerprints of a container in a series, which increases the caching.
         *
         * Being a gc candidate does not mean that the chunk will be removed because of other log entries that
         * will increase the reference counter and because of concurrent requests. The gc has to recheck the
        ...
        /**
         * if true the Garbage Collector is paused and will not start processing in idle time
         */
         ...
         * The condition system is used to prevent that state changed are only recognized after at most
        ...
        /**
         * maximal time in seconds that the gc might use for processing a gc candidate container.
         */
         ...
         * log id of the matching block mapping written event. This set is used to ensure
         * exactly-once semantics for block mapping failed events.
         *
         * The data structure has to be persisted between calls. The data structure is cleared when
         * the log is empty.
         */
         ...
         */
        bool ProcessGCCandidate(GarbageCollectionCandidateData* candidate_data, bool* changed);

        /**
         * Core handling of the candidate processing.
         * The interaction of the candidate processing with the candidate info database is done
         * in the ProcessGCCandidate that usually calls this method.
         */
        bool DoProcessGCCandidate(GarbageCollectionCandidateData* candidate_data, bool* changed);
         ...
         * @return true iff ok, otherwise an error has occurred
         */
        bool ProcessGCCandidateItem(const GarbageCollectionCandidateData& candidate_data,
                const GarbageCollectionCandidateItemData& item, std::pair<bool, bool>* del);
         ...
         */
        bool
        ProcessOphranChunks(const OphranChunksEventData& event_data, const dedupv1::log::LogReplayContext& context);

        enum dedupv1::base::lookup_result ProcessGCCandidates();

        bool ProcessBlockMapping(const dedupv1::blockindex::BlockMappingPair& mapping_pair,
    ...
    public:
         ...
         * dedupv1 check
         */
        virtual dedupv1::base::Option<bool> IsGCCandidate(uint64_t address, const void* fp, size_t fp_size);

        /**
         * returns the gc candidate info
         * @return
         */
        inline dedupv1::base::PersistentIndex* candidate_info();
         ...
         * @return true iff ok, otherwise an error has occurred
         */
        bool Put(const std::multimap<uint64_t, dedupv1::chunkindex::ChunkMapping>& gc_chunks, bool failed_mode);
