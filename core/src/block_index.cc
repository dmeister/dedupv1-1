...
bool BlockIndex::WriteDeleteLogEntry(const BlockMapping& previous_block_mapping,
    ...
    CHECK(log_->CommitEvent(EVENT_TYPE_BLOCK_MAPPING_DELETED, &event_data, ec).valid(),
...
Option<int64_t> BlockIndex::WriteLogEvent(
    ...
    CHECK(updated_block_mapping.SerializeTo(event_data.mutable_mapping(), true, true),
      "Cannot serialize mapping: " <<
        updated_block_mapping.DebugString());

    Option<int64_t> commit_result = log_->CommitEvent(EVENT_TYPE_BLOCK_MAPPING_WRITTEN, &event_data, ec);
...
bool BlockIndex::StoreBlock(const BlockMapping& previous_block_mapping,
    ...
    Option<int64_t> log_result = WriteLogEvent(
        updated_block_mapping, ec);
    if (!log_result.valid()) {
            ERROR("Failed to write block index log entry: " <<
                "mapping " << updated_block_mapping.DebugString());
            return false;
    }

    BlockMappingData block_mapping_data;
    CHECK(updated_block_mapping.SerializeTo(&block_mapping_data, false, true), 
        "Cannot serialize block mapping: " <<
        updated_block_mapping.DebugString());
    block_mapping_data.set_event_log_id(log_result.value());
...
delete_result BlockIndex::DeleteBlockInfo(uint64_t block_id,
    ...
    CHECK_RETURN(WriteDeleteLogEntry(block_mapping, ec), DELETE_ERROR,
...
bool BlockIndex::LogReplayBlockMappingWrittenDirty(dedupv1::log::replay_mode replay_mode,
    ...
    CHECK(event_data.has_mapping(), "Event data has no block mapping: " << event_data.ShortDebugString());
    BlockMapping stored_mapping(block_size_);
    CHECK(stored_mapping.CopyFrom(event_data.mapping()),
        "Cannot copy from stored block mapping: " << event_data.mapping().ShortDebugString());
    ...
    DEBUG("Replay block mapping written (dirty): " << stored_mapping.DebugString() << "\n" <<
...
bool BlockIndex::LogReplayBlockMappingWrittenBackground(dedupv1::log::replay_mode replay_mode,
    ...
    BlockMapping stored_mapping(this->block_size_);
    CHECK(stored_mapping.CopyFrom(event_data.mapping()),
        "Cannot copy from stored block mapping: " << event_data.mapping().ShortDebugString());
    ...
    if (aux_lr == LOOKUP_NOT_FOUND) {
        TRACE("Skip replay block mapping written: " << stored_mapping.DebugString() << "\n" <<
            " replay mode " << dedupv1::log::Log::GetReplayModeName(replay_mode) <<
            ", event log id " << context.log_id() << "\n" <<
            (aux_lr == LOOKUP_FOUND ? ", auxiliary " + aux_mapping.DebugString() : ", auxiliary <not found>") );
        return true;
    ...
    DEBUG("Replay block mapping written: " << stored_mapping.DebugString() << "\n" <<
        " replay mode " << dedupv1::log::Log::GetReplayModeName(replay_mode) <<
        ", event log id " << context.log_id() << "\n" <<
        (lr == LOOKUP_FOUND ? ", persistent " + current_mapping.DebugString() : ", persistent <not found>") << "\n" <<
        (aux_lr == LOOKUP_FOUND ? ", auxiliary " + aux_mapping.DebugString() : ", auxiliary <not found>") );
...
dedupv1::base::Option<bool> BlockIndex::TryImportBlock(bool with_waiting_period) {
    ...
    if (with_waiting_period) {
        ...
        if (waiting_period > 0) {
            ...
            ThreadUtil::Sleep(waiting_period, dedupv1::base::timeunit::MILLISECONDS);
        }
    }
