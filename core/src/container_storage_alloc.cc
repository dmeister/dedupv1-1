...
bool ContainerStorageAllocator::OnCommitContainer(const Container& container, 
...
bool MemoryBitmapContainerStorageAllocator::SetOption(const string& option_name, const string& option) {
    ...
    if (option_name == "type") {
        ...
        CHECK(this->persistent_bitmap_->SetOption("max-key-size", "8"), 
            "Failed to set auto option");
        return true;
    }
    CHECK(this->persistent_bitmap_, "Bitmap not set");
    CHECK(this->persistent_bitmap_->SetOption(option_name, option), 
...
bool MemoryBitmapContainerStorageAllocator::Store(int file_index) {
    ...
    TRACE("Store bitmap: file " << file_index << 
        ", free places " << this->file_[file_index].bitmap_->clean_bits());

    CHECK(this->file_[file_index].bitmap_->Store(false), 
        "Unable to store bitmap file " << file_index);
...
bool MemoryBitmapContainerStorageAllocator::Start(const StartContext& start_context, 
    ...
    file_locks_.Resize(storage_->GetFileCount());
    for (uint32_t i = 0; i < storage_->GetFileCount(); i++) {
        size_t file_size = storage_->file(i).file_size();
        size_t container_in_file_ = file_size / storage_->container_size();

        Bitmap* bitmap = new Bitmap(container_in_file_);
        CHECK(bitmap->setPersistence(persistent_bitmap_, &i, sizeof(uint32_t), page_size_), 
            "Could not set persistency for Bitmap " << i);
...
bool MemoryBitmapContainerStorageAllocator::Run() {
    ...
    // During Crash Restart we loose the real number of clean bits, Therefore 
    // we repair this here
    free_count_ = 0;
...
bool MemoryBitmapContainerStorageAllocator::MarkAddressUsed(
    const ContainerStorageAddressData& address) {
    int file_index = address.file_index();
    CHECK(file_index >= 0 && file_index < file_.size(), 
        "Illegal file index: " << address.ShortDebugString());

    CHECK(address.file_offset() % storage_->container_size() == 0, 
        "Illegal container offset: " << address.ShortDebugString());

    DCHECK(file_index >= 0 && file_index < file_.size(), "Illegal file index");
    uint64_t bit_offset = address.file_offset() / storage_->container_size();

    ScopedLock file_lock(file_locks_.Get(file_index));
    CHECK(file_lock.AcquireLock(), "Failed to acquire file lock: file index " << file_index << 
    ...
    CHECK(file_[file_index].bitmap_->set(bit_offset), 
        "Could not set bit " << bit_offset << " of bitmap " << file_index);

    CHECK(file_lock.ReleaseLock(), "Failed to release file lock");
    return true;
}

bool MemoryBitmapContainerStorageAllocator::FreeAddress(
    const ContainerStorageAddressData& address)
{
    ...
    ScopedLock file_lock(file_locks_.Get(file_index));
    CHECK(file_lock.AcquireLock(), 
        "Failed to acquire file lock: file index " << file_index << 
        ", file size " << file_.size());

    DCHECK(bit_offset < file_[file_index].bitmap_->size(), 
        "Invalid bit offset " << bit_offset << " for bitmap " << file_index);

    CHECK(file_[file_index].bitmap_->clear(bit_offset), 
        "Unable to clear bit " << bit_offset << " of bitmap " << file_index);
...
uint64_t MemoryBitmapContainerStorageAllocator::GetActiveStorageDataSize() {
    ...
    return used_container_places * this->storage_->container_size();
}
...
bool MemoryBitmapContainerStorageAllocator::SearchFreeAddress(int file_index, 
    ...
    CHECK(file_[file_index].bitmap_->set(pos.value()), 
        "Could not set clear found pos " << pos.value() << " if bitmap " << file_index);
    free_count_--;
    ...
    new_address->set_file_offset(storage_->container_size() * pos.value());
...
bool MemoryBitmapContainerStorageAllocator::OnAbortContainer(const Container& container,
    ...
    CHECK(FreeAddress(address), "Failed to free address " << address.ShortDebugString());
    return true;
}
...
bool MemoryBitmapContainerStorageAllocator::EnsurePagePersisted(const ContainerStorageAddressData& address) {
    ...
    DCHECK(file_index < file_.size(), "Illegal file index: " << file_index);
    DCHECK((file_offset % storage_->container_size()) == 0, 
        "Invalid file offset " << file_offset << " for container size " << storage_->container_size());

    uint64_t item_index = file_offset / storage_->container_size();
    ...
    DEBUG("Ensure that persisted: file index " << file_index << 
...
bool MemoryBitmapContainerStorageAllocator::LogReplay(dedupv1::log::event_type event_type,
    ...
    if (context.replay_mode() == dedupv1::log::EVENT_REPLAY_MODE_REPLAY_BG) {
        ...
        if (event_type == dedupv1::log::EVENT_TYPE_CONTAINER_COMMITED) {
            ...
            CHECK(MarkAddressUsed(event_data.address()),
                "Failed to mark address as used: " << event_data.ShortDebugString());
        } else if (event_type == dedupv1::log::EVENT_TYPE_CONTAINER_DELETED) {
            ContainerDeletedEventData event_data = event_value.container_deleted_event();

            CHECK(FreeAddress(event_data.address()), "Failed to free address: " <<
                event_data.ShortDebugString());
        } else if (event_type == dedupv1::log::EVENT_TYPE_CONTAINER_MERGED) {
            ContainerMergedEventData event_data = event_value.container_merged_event();

            CHECK(FreeAddress(event_data.first_address()), "Failed to free address: " <<
                event_data.ShortDebugString());
            CHECK(FreeAddress(event_data.second_address()), "Failed to free address: " <<
                event_data.ShortDebugString());
            CHECK(MarkAddressUsed(event_data.new_address()),
                "Failed to mark address as used: " << event_data.ShortDebugString());
        } else if (event_type == dedupv1::log::EVENT_TYPE_CONTAINER_MOVED) {
            ContainerMoveEventData event_data = event_value.container_moved_event();

            CHECK(FreeAddress(event_data.old_address()), "Failed to free address: " <<
                event_data.ShortDebugString());
            CHECK(MarkAddressUsed(event_data.new_address()),
                "Failed to mark address as used: " << event_data.ShortDebugString());
        }
...
bool MemoryBitmapContainerStorageAllocator::OnCommitContainer(const Container& container,
    const ContainerStorageAddressData& address) {

    DEBUG("Container commit address handling: " <<
        "container " << container.DebugString() <<
        ", address " << address.ShortDebugString());

    CHECK(MarkAddressUsed(address),
      "Failed to mark address as used: " << address.ShortDebugString());
    return true;
}
...
enum alloc_result MemoryBitmapContainerStorageAllocator::OnNewContainer(const Container& container,
    ...
    for (int i = 0; i < file_.size(); i++) {
        ...
        ScopedLock file_lock(file_locks_.Get(file_index));
        CHECK_RETURN(file_lock.AcquireLock(), ALLOC_ERROR, 
            "Failed to acquire file lock: file index " << file_index <<
            ", file count " << file_.size());

        TRACE("Test file: " << file_index << ", free count " << file_[file_index].bitmap_->clean_bits());

        // The full check always leaved one container place free per file unless 
        // the operation is for a merge or delete item operation
        // This avoid being locked in a situation where there is no space left, 
        // but tons of gc-able containers, but we fail to free the space
        ...
        if (file_is_full) {
            CHECK_RETURN(file_lock.ReleaseLock(), ALLOC_ERROR, 
                "Failed to release file lock");
            continue;
        }
        found_file = true;

        CHECK_RETURN(SearchFreeAddress(file_index, new_address), ALLOC_ERROR, 
            "Failed to search free address");
        CHECK_RETURN(file_lock.ReleaseLock(), ALLOC_ERROR, 
            "Failed to release file lock");
        break;
    }
...
bool MemoryBitmapContainerStorageAllocator::OnMerge(const ContainerMergedEventData& data) {
    ...
    CHECK(FreeAddress(data.first_address()), 
        "Failed to free address " << data.first_address().ShortDebugString()); FAULT_POINT("container-storage.alloc.merge.after-first-free");

    CHECK(FreeAddress(data.second_address()), 
        "Failed to free address " << data.second_address().ShortDebugString()); FAULT_POINT("container-storage.alloc.merge.after-second-free");
    return true;
}
...
bool MemoryBitmapContainerStorageAllocator::OnDeleteContainer(const ContainerDeletedEventData& data) {
    ...
    CHECK(FreeAddress(data.address()), "Failed to free address " << data.address().ShortDebugString());
    return true;
}
...
bool MemoryBitmapContainerStorageAllocator::OnMove(const ContainerMoveEventData& data) {
    ...
    CHECK(FreeAddress(data.old_address()), 
        "Failed to free address " << data.old_address().ShortDebugString());
    return true;
}
