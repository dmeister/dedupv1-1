...
{
    return address.has_primary_id() ||
           (address.has_file_index() && address.has_file_offset());
}
...
public:
    ...
    bool Insert(list<uint64_t>& container_id_list)
    {
        ...
        for (i = container_id_list.begin(); i != container_id_list.end();
             i++) {
    ...
    void RemoveAllFromSet()
    {
        ...
        list<uint64_t>::iterator i;
        for (i = locked_container_id_list_.begin();
             i != locked_container_id_list_.end();
             i++) {
...
Option<bool> ContainerStorage::TryReadContainerFromWriteCache(Container* container)
{
    Container* write_container = NULL;
    ReadWriteLock* write_cache_lock = NULL;
    lookup_result r = write_cache_.GetWriteCacheContainer(container->primary_id(),
        &write_container,
        &write_cache_lock,
        false);
    CHECK(r != LOOKUP_ERROR,
        "Failed to access write cache");
    if (r == LOOKUP_FOUND) {
        ...
        bool copy_result = container->CopyFrom(*write_container, true);
...
lookup_result ContainerStorage::ReadContainer(Container* container,
    bool write_cache)
{
    ...
    DEBUG("Read container (with cache): " << container->DebugString());
    ...
    CacheEntry cache_entry;
      ...
      lookup_result r = this->cache_.CheckCache(
        container->primary_id(), &cache_container, !use_cache, false,
        &cache_entry);
      CHECK_RETURN(r != LOOKUP_ERROR,
        LOOKUP_ERROR,
        "Failed to check cache: container " << container->DebugString());
      ...
      if (r == LOOKUP_FOUND) {
        ...
        TRACE("Read container: container " << container->DebugString() <<
            " (read cache) ");
    ...
    if (write_cache) {
      // we now check the write cache
      Option<bool> write_cache_check = TryReadContainerFromWriteCache(container);
      if (!write_cache_check.valid()) {
          if (use_cache && cache_entry.is_set()) {
              // do not copy write cache data to the read cache
              CHECK_RETURN(cache_.ReleaseCacheline(
                    container->primary_id(), &cache_entry), LOOKUP_ERROR,
              "Failed to release cache line");
          }

        ERROR("Failed to check write cache");
        return LOOKUP_ERROR;
      } else if (write_cache_check.value()) {
          if (use_cache && cache_entry.is_set()) {
              // do not copy write cache data to the read cache
              CHECK_RETURN(cache_.ReleaseCacheline(
                    container->primary_id(), &cache_entry), LOOKUP_ERROR,
    ...
    // container is also not in write cache
    enum lookup_result b = ReadContainerFromDisk(container); // now container lock held
    if (use_cache && cache_entry.is_set()) {
        if (b == LOOKUP_ERROR || b == LOOKUP_NOT_FOUND) {
            ...
            CHECK_RETURN(cache_.ReleaseCacheline(
                    container->primary_id(), &cache_entry), LOOKUP_ERROR,
                "Failed to release cache line");
        } else if (!this->cache_.CopyToReadCache(*container, &cache_entry)) {
            ERROR(
                "Failed to add container to read cache: " <<
...
pair<lookup_result,
    ContainerStorageAddressData> ContainerStorage::LookupContainerAddressNoWait(
    uint64_t container_id,
    ReadWriteLock**
    primary_container_lock,
    bool acquire_write_lock)
{
    DCHECK_RETURN(state_ == STOPPING || state_ == RUNNING || state_ == STARTED,
        make_pair(LOOKUP_ERROR,
            ContainerStorageAddressData()),
        "Illegal state to lookup container: container " << container_id <<
        ", state " <<
        this->state_);
    DCHECK_RETURN(this->meta_data_index_,
        make_pair(LOOKUP_ERROR,
            ContainerStorageAddressData()), "Meta data index not set");

    TRACE("Lookup container address (no wait): container id " << container_id);

    ScopedReadWriteLock scoped_lock(&this->meta_data_lock_);
    CHECK_RETURN(scoped_lock.AcquireReadLock(),
        make_pair(LOOKUP_ERROR, ContainerStorageAddressData()),
    ...
    lookup_result lr =
        this->meta_data_index_->Lookup(&container_id, sizeof(uint64_t),
            &container_address);
    if (lr == LOOKUP_ERROR || lr == LOOKUP_NOT_FOUND) {
        TRACE("No meta data entry for container: container id " << container_id);
        return make_pair(lr, container_address);
    }
    // lr == FOUND
    DCHECK_RETURN(IsValidAddressData(container_address),
        make_pair(LOOKUP_ERROR,
            container_address), "Invalid address data: " <<
    ...
    if (container_address.has_primary_id()) {
        ...
        TRACE(
            "Lookup primary container address: container id " <<
            container_id <<
            ", primary container id " << primary_id);

        lr = this->meta_data_index_->Lookup(&primary_id, sizeof(uint64_t),
            &container_address);
        if (lr == LOOKUP_ERROR) {
            ERROR(
                "Failed to lookup primary id: secondary id " <<
                container_id <<
                ", address " << container_address.ShortDebugString() <<
                ", first address looked up " <<
                first_lookedup_container_address.ShortDebugString());
        }
        if (lr == LOOKUP_ERROR || lr == LOOKUP_NOT_FOUND) {
            return make_pair(lr, container_address);
        }
        ...
        if (!container_address.has_primary_id()) {
            container_address.set_primary_id(primary_id); // reset the value to show that the lookup
        }
        // was redirected
        else{
            // if the address has a primary id, we are in trouble. This case is handled as error in
            // a few lines.
        }
        // lr == FOUND
    }
    CHECK_RETURN(
        container_address.has_file_index() &&
        container_address.has_file_offset(),
        make_pair(LOOKUP_ERROR, container_address),
        "Illegal container address: " <<
        "address " << container_address.ShortDebugString() <<
        ", lookup container id " << container_id <<
        ", first looked up container address " <<
        first_lookedup_container_address.ShortDebugString() <<
...
pair<lookup_result,
    ContainerStorageAddressData> ContainerStorage::LookupContainerAddress(
    uint64_t container_id,
    ReadWriteLock**
    primary_container_lock,
    bool
    acquire_write_lock)
{
    ...
    pair<lookup_result, ContainerStorageAddressData> container_address =
        LookupContainerAddressNoWait(container_id,
            primary_container_lock,
            acquire_write_lock);
    ...
    if (container_address.first == LOOKUP_NOT_FOUND) {
        ...
        TRACE(
            "Container not found in meta data index: container id " <<
            container_id);

        // container not in the write cache, but in the background committer thing
        Option<bool> is_processed =
            this->background_committer_.IsCurrentlyProcessedContainerId(
                container_id);
        while (is_processed.valid() && is_processed.value()) {
            TRACE(
                "Wait until currently processed container is committed: container "
                <<
                container_id);
            dedupv1::base::timed_bool b =
                this->background_committer_.CommitFinishedConditionWaitTimeout(
                    1,
                    dedupv1::base::timeunit::SECONDS);
            CHECK_RETURN(b != TIMED_FALSE,
                make_pair(LOOKUP_ERROR,
                    ContainerStorageAddressData()),
                "Failed to wait for commit");
            if (b == TIMED_TIMEOUT) {
                TRACE(
                    "Wait for container commit timeout: container " <<
                    container_id);
            }
        ...
        CHECK_RETURN(is_processed.valid(),
            make_pair(LOOKUP_ERROR, ContainerStorageAddressData()),
            "Failed to check processing state");
        TRACE(
            "Finished waiting for currently processed container: container " <<
            container_id);

        // re-lookup
        container_address = LookupContainerAddressNoWait(container_id,
            primary_container_lock,
            acquire_write_lock);
        CHECK_RETURN(container_address.first != LOOKUP_ERROR,
            container_address,
            "Failed to lookup container address: " << container_id);
        if (container_address.first == LOOKUP_NOT_FOUND) {
...
enum lookup_result ContainerStorage::ReadContainerFromDiskLocked(
    Container* container,
    const ContainerStorageAddressData&
    container_address){
    DCHECK_RETURN(container, LOOKUP_ERROR, "Container not set");
    DCHECK_RETURN(state_ == STOPPING || state_ == RUNNING || state_ == STARTED, LOOKUP_ERROR,
    ...
    CHECK_RETURN(
        file_index < this->file_.size(),
        LOOKUP_ERROR, "Illegal file index: " << file_index <<
        ", file count " <<
        this->file_.size());
    ...
    CHECK_RETURN(
        file,
        LOOKUP_ERROR,
        "File not open: file index: " << file_index << ", file count " <<
        this->file_.size());
    ...
    CHECK_RETURN(file_lock.AcquireLockWithStatistics(
            ...
            &this->stats_.file_lock_busy_), LOOKUP_ERROR,
        "Failed to acquire file lock: " <<
        "file index " << file_index <<
        ", container " << container->DebugString());
    file_lock_timer.stop();

    bool load_file = false;
    ProfileTimer load_file_timer(this->stats_.total_file_load_time_);
    tbb::tick_count load_start = tbb::tick_count::now();
    load_file = container->LoadFromFile(file,
        file_offset);
    stats_.average_container_load_latency_.Add(
        (tbb::tick_count::now() - load_start).seconds() * 1000);
    load_file_timer.stop();
    if (!load_file) {
...
enum lookup_result ContainerStorage::ReadContainerFromDisk(Container* container){
    DCHECK_RETURN(container, LOOKUP_ERROR, "Container not set");
    DCHECK_RETURN(state_ == STOPPING || state_ == RUNNING || state_ == STARTED,
        LOOKUP_ERROR,
        "Illegal state to read container: " << this->state_);
    ...
    pair<lookup_result, ContainerStorageAddressData> container_address =
        LookupContainerAddress(id, &lock, false);
    ...
    return ReadContainerFromDiskLocked(container,
        container_address.second);
}

bool ContainerStorage::WriteContainer(Container* container,
    const ContainerStorageAddressData& container_address)
{
    DCHECK(container, "Container not set");
    DCHECK(
        container->primary_id() != 0, "Container id not set: " <<
        container->DebugString());
    DCHECK(
        container->primary_id() != Storage::ILLEGAL_STORAGE_ADDRESS,
        "Illegal container id " + container->primary_id());
    DCHECK(state_ == STOPPING || state_ == RUNNING || state_ == STARTED,
        "Illegal state to write container: container id " <<
    ...
    // TODO(fermat): Is this lock needed any more? It should be no problem to access the same file
    // at different positions concurrently.
    ScopedLock file_lock(this->file_[file_index].lock());
    CHECK(file_lock.AcquireLockWithStatistics(
            &this->stats_.file_lock_free_,
            &this->stats_.file_lock_busy_),
        "Failed to acquire file lock: file index " <<
        file_index);

    CHECK(container->StoreToFile(file,
            file_offset),
        "Cannot write container " << container_id << ": " <<
        container->DebugString());
    CHECK(file_lock.ReleaseLock(), "Container unlock failed");
    FAULT_POINT("container-storage.write.after-write");
    TRACE("Write container: " << container->DebugString() << ", address " <<
        DebugString(container_address));
...
bool ContainerStorage::MarkContainerCommitAsFailed(Container* container)
{
    ...
    Option<int64_t> commit_result = log_->CommitEvent(EVENT_TYPE_CONTAINER_COMMIT_FAILED,
        &failed_data, NO_EC);
    CHECK(commit_result.valid(),
        "Failed to commit container: " << failed_data.ShortDebugString());
    CHECK(PostContainerEvent(*container,
          EVENT_TYPE_CONTAINER_COMMIT_FAILED,
          &failed_data,
          commit_result.value()),
...
bool ContainerStorage::CommitContainer(Container* container,
    const ContainerStorageAddressData& address)
{
    ...
    DCHECK(
        ...
        "Illegal container id: " + container->primary_id());
    DCHECK(state_ == STOPPING || state_ == RUNNING || state_ == STARTED,
        "Illegal state to commit container: " <<
    ...
    TRACE(
        "Committing container " << container_id << ": " <<
        container->DebugString());

    // Lookup for security
    // we also could use IsCommitted here, but we get more information about all the stuff if we do
    // it directly
    pair<lookup_result,
        ContainerStorageAddressData> result = this->LookupContainerAddressNoWait(
        container_id,
        NULL,
        false);
    CHECK(result.first != LOOKUP_ERROR,
        "Meta data lookup failed: container " << container->DebugString());
    if (result.first == LOOKUP_FOUND) {
        ...
        CHECK(MarkContainerCommitAsFailed(
                container),
            "Failed to mark container commit as failed: " <<
            container->DebugString());
        return false;
    }

    DEBUG("Commit container: " << container->DebugString() <<
        ", " << DebugString(address));
    ...
    this->stats_.committed_container_++;
    ...
    Option<int64_t> commit_result = log_->CommitEvent(EVENT_TYPE_CONTAINER_COMMITED,
        &event_data, NO_EC);
    if (!commit_result.valid()) {
        ERROR(
            "Commit log entry about container commit failed: container " <<
            container->DebugString());
        if (event_log_id != 0) {
            ...
            CHECK(log_->CommitEvent(EVENT_TYPE_CONTAINER_COMMIT_FAILED,
                    &failed_data,
                    NO_EC).valid(),
                "Failed to commit container: " << failed_data.ShortDebugString());
        }
...
bool ContainerStorage::PostContainerEvent(
    const Container& container,
    event_type event_type,
    const Message* log_message,
    uint64_t log_id)
{
    TRACE("Post container event: " << dedupv1::log::Log::GetEventTypeName(event_type) <<
    ...
    if (event_type == EVENT_TYPE_CONTAINER_COMMITED) {
        ...
        const ContainerCommittedEventData* event_message =
            dynamic_cast<const ContainerCommittedEventData*>(log_message);
        ...
        while (!updated) {
            ...
            if (container_id > hccid) {
                ...
                if (highest_committed_container_id_.compare_and_swap(
                        container_id,
                        hccid) == hccid) {
        ...
        // Update log id
        event_data.mutable_address()->set_log_id(log_id);

        // Update index
        DCHECK(IsValidAddressData(
                event_data.address()), "Invalid address data: " <<
            event_data.ShortDebugString());
        CHECK(this->meta_data_index_->Put(&container_id,
                sizeof(uint64_t),
                event_data.address()),
        ...
        if (!gc_->OnCommit(event_data)) {
            WARNING(
                "Error while updating storage gc: " <<
                event_data.ShortDebugString());
        }

        if (!allocator_->OnCommitContainer(container,
              event_data.address())) {
            WARNING("Error while updating allocation state");
        }

        // Container Moved
    }else if (event_type == EVENT_TYPE_CONTAINER_MOVED) {
        CHECK(log_message, "Log message not set");
        const ContainerMoveEventData* event_message =
            dynamic_cast<const ContainerMoveEventData*>(log_message);
        CHECK(event_message, "Log message not set")
        ...
        // Update log id
        event_data.mutable_new_address()->set_log_id(log_id);
        ...
        DCHECK(IsValidAddressData(
                event_data.new_address()), "Invalid address data: " <<
            event_data.ShortDebugString());
        DCHECK(IsValidAddressData(
                event_data.old_address()), "Invalid address data: " <<
            event_data.ShortDebugString());

        CHECK(this->meta_data_index_->Put(&container_id, sizeof(uint64_t),
                event_data.new_address()),
            "Meta data update failed: " << container_id <<
            ", address " << DebugString(event_data.new_address()));

        ScopedReadWriteLock scoped_container_lock(this->GetContainerLock(
                event_data.container_id()));
        CHECK(
            scoped_container_lock.AcquireWriteLock(),
        ...
        if (this->allocator_) {
            if (!this->allocator_->OnMove(event_data)) {
                WARNING(
                    "Failed to update storage allocator after container move: "
                    <<
                    event_data.ShortDebugString());
            }
        }
        ...
        CHECK(
            scoped_container_lock.ReleaseLock(),
            "Failed to release container lock");

        // Container Merged
    }else if (event_type == EVENT_TYPE_CONTAINER_MERGED) {
        CHECK(log_message, "Log message not set");
        const ContainerMergedEventData* event_message =
            dynamic_cast<const ContainerMergedEventData*>(log_message);
        CHECK(event_message, "Log message not set")
        ...
        // Update index
        TRACE(
            "Redirect primary id " << container_id << ": " <<
            event_data.new_address().ShortDebugString());
        ...
        secondary_data_address.set_log_id(log_id);

        for (int i = 0; i < event_data.new_secondary_id_size(); i++) {
            uint64_t id = event_data.new_secondary_id(i);
            TRACE(
                "Redirect merged container " << id << ": primary id " <<
                secondary_data_address.primary_id());
            DCHECK(IsValidAddressData(
                    secondary_data_address), "Invalid address data: " <<
                secondary_data_address.ShortDebugString());
            CHECK(this->meta_data_index_->Put(&id, sizeof(id),
                    secondary_data_address),
                "Cannot write new container address: container id " << id);
            FAULT_POINT(
                "container-storage.ack.container-merge-after-secondary-put");
        }
        for (int i = 0; i < event_data.unused_ids_size(); i++) {
            ...
            CHECK(this->meta_data_index_->Delete(&id,
                    sizeof(id)),
                "Cannot delete unused container address: container id " << id);
        ...
        ScopedReadWriteLock container_lock1(this->GetContainerLock(
                event_data.first_id()));
        CHECK(
            container_lock1.AcquireWriteLock(),
            "Cannot acquire container write lock: container id " <<
            event_data.first_id());
        ScopedReadWriteLock container_lock2(this->GetContainerLock(
                event_data.second_id()));
        if (container_lock2.Get() != container_lock1.Get()) {
            CHECK(
                container_lock2.AcquireWriteLock(),
                "Cannot acquire container write lock: container id " <<
                event_data.second_id());
        }
        ScopedReadWriteLock container_lock3(this->GetContainerLock(
                event_data.new_primary_id()));
        if (container_lock3.Get() != container_lock1.Get() &&
            container_lock3.Get() !=
            container_lock2.Get()) {
            CHECK(
                container_lock3.AcquireWriteLock(),
                "Cannot acquire container write lock: container id " <<
                event_data.new_primary_id());
        }
        CHECK(scoped_lock.ReleaseLock(), "Failed to release meta data lock");
        ...
        // No client is allowed to read the container on the old location after the allocator freed
        // the space
        // We ensure this by acquiring the container lock while the meta data lock is held.
        // If any client has started a container read (with acquing the meta data lock) it is
        // finished and
        // has released the meta data lock and the container lock when the thread is here.

        if (this->allocator_) {
            CHECK(this->allocator_->OnMerge(
                    event_data),
                "Cannot get merge container address: " <<
                event_data.ShortDebugString());
        }
        if (this->gc_) {
        ...
        CHECK(
            container_lock1.ReleaseLock(),
            "Cannot release container write lock");
        if (container_lock2.Get() != container_lock1.Get()) {
            CHECK(
                container_lock2.ReleaseLock(),
                "Cannot release container write lock");
        }
        if (container_lock3.Get() != container_lock1.Get() &&
            container_lock3.Get() !=
            container_lock2.Get()) {
            CHECK(
                container_lock3.ReleaseLock(),
                "Cannot release container write lock");
        }
    }else if (event_type == EVENT_TYPE_CONTAINER_DELETED) {
        CHECK(log_message, "Log message not set");
        const ContainerDeletedEventData* event_message =
            dynamic_cast<const ContainerDeletedEventData*>(log_message);
        CHECK(event_message, "Log message not set")
        ...
        for (set<uint64_t>::const_iterator i = all_old_ids.begin();
             i != all_old_ids.end();
             i++) {
            uint64_t id = *i;
            TRACE("Delete unused container id: container " << id);
        ...
        ScopedReadWriteLock container_lock(this->GetContainerLock(
                event_data.container_id()));
        CHECK(
            container_lock.AcquireWriteLock(),
            "Cannot acquire container write lock");

        CHECK(scoped_lock.ReleaseLock(), "Failed to release meta data lock");

        // Ok, there the probability of a race condition is non-existing because there should be 0
        // (in words zero)
        // clients accessing this container. It is gone. Deal with it.

        if (this->allocator_) {
            CHECK(this->allocator_->OnDeleteContainer(
                    event_data),
                "Cannot get delete container address: " <<
                event_data.ShortDebugString());
        }
...
bool ContainerStorage::CheckForPartialContainerFileCleanup() {
    bool do_flush = false;
    bool do_exit = false;
    for (int i = 0; i < partial_file_.size() && !do_exit; i++) {
        ContainerFile& partial_container_file(partial_file_[i]);
        CHECK(partial_container_file.lock()->AcquireLock(),
            "Failed to acquire partial container file lock");

        TRACE("Check: " << i << ", offset " << partial_file_offset_[i] <<
            ", threshold " << partial_file_size_threshold_);
        if (partial_file_offset_[i] >= partial_file_size_threshold_) {
            int sleep_count = 0;
            uint64_t last_possible_container_in_file = last_given_container_id_;
            while (highest_committed_container_id_ < last_possible_container_in_file &&
                   sleep_count < 8) {
                ThreadUtil::Sleep(1, dedupv1::base::timeunit::SECONDS);
                sleep_count++;
            }
            if (sleep_count >= 8) {
                INFO("Skip truncate after waiting: " <<
                    "last possible container id in file " << last_possible_container_in_file <<
                    ", highest committed container id " << highest_committed_container_id_ <<
                    ", partial file " << partial_container_file.filename());
                do_flush = true;
            } else {
                DEBUG("Truncate partial container file " <<
                    partial_container_file.filename());
                stats_.partial_container_file_truncate_count_++;
                // we are now save
                if (!partial_container_file.file()->Truncate(kSuperBlockSize)) {
                    ERROR("Failed to truncate partial container file");
                    do_exit = true;
                }
                partial_file_offset_[i] = kSuperBlockSize;
            }
        }

        CHECK(partial_container_file.lock()->ReleaseLock(),
            "Failed to release partial container file lock");
    }

    // poor mans timeout. This ensures progress
    if (do_flush) {
        Flush(NO_EC);
    }
    return true;
...
ContainerStorage::ContainerStorage() :
    partial_commit_cleanup_thread_(
        NewRunnable(this, &ContainerStorage::PartialCommitCleanupRunner),
        "partial commit cleanup"),
    cache_(this),
    write_cache_(this)
{
    ...
    partial_file_size_threshold_ = 32 * 1024 * 1024;
#ifdef DEDUPV1_CORE_TEST
...
ContainerStorage::Statistics::Statistics() : average_container_load_latency_(16)
{
    ...
    partial_container_file_truncate_count_ = 0;
}

bool ContainerStorage::SetOption(const string& option_name,
    const string& option)
{
    ...
    if (StartsWith(option_name, "background-commit.")) {
        CHECK(this->background_committer_.SetOption(option_name.substr(strlen(
                        "background-commit.")),
                option), "Config failed");
        return true;
    }
    if (option_name == "compression") {
        CHECK(this->compression_ == NULL, "Compression already set");
        if (option == "deflate") {
            this->compression_ = Compression::NewCompression(
                Compression::COMPRESSION_ZLIB_1);
            return true;
        }else if (option == "bz2") {
            this->compression_ = Compression::NewCompression(
    ...
    if (option_name == "filename") {
        ...
        for (int i = 0; i < this->file_.size(); i++) {
            CHECK(
                this->file_[i].filename() != option,
                "Double container file: " <<
                option);
        }
    ...
    if (option_name == "partial.filename") {
        CHECK(option.size() <= 255, "Filename too long");
        CHECK(option.size() > 0, "Filename too short");

        int next_index = partial_file_.size();
        partial_file_.resize(partial_file_.size() + 1); // new entry
        partial_file_[next_index].Init(option);
        return true;
    }
    if (option_name == "filesize") {
        ...
        CHECK(ToStorageUnit(option).valid(), "Illegal option " << option);
        CHECK(ToStorageUnit(
                option).value() >= 0, "Illegal file size " << option);
    ...
    if (StartsWith(option_name, "meta-data.")) {
        ...
        CHECK(this->meta_data_index_->SetOption(option_name.substr(strlen(
                        "meta-data.")),
                option), "Config failed");
        return true;
    }
    if (StartsWith(option_name, "write-cache.")) {
        CHECK(this->write_cache_.SetOption(option_name.substr(strlen(
                        "write-cache.")),
                option), "Config failed");
        return true;
    }
    if (StartsWith(option_name, "read-cache.")) {
        CHECK(this->cache_.SetOption(option_name.substr(strlen(
                        "read-cache.")),
                option), "Config failed");
        return true;
    }
    ...
    if (StartsWith(option_name, "gc.")) {
        ...
        CHECK(this->gc_->SetOption(option_name.substr(strlen(
                        "gc.")),
                option), "Configuration failed: " << option_name <<
            " - " <<
            option);
        return true;
    }
    ...
    if (StartsWith(option_name, "alloc.")) {
        ...
        CHECK(this->allocator_->SetOption(option_name.substr(strlen("alloc.")),
                option),
...
bool ContainerStorage::OpenPartialContainerFiles(const ContainerLogfileData& log_data,
  const StartContext& start_context) {

    if (partial_file_.empty()) {
        INFO("Auto-configure partial container files");
        for (int i = 0; i < file_.size(); i++) {
            int next_index = partial_file_.size();
            partial_file_.resize(partial_file_.size() + 1); // new entry
            partial_file_[next_index].Init(file_[i].filename() + ".partial");
        }
    }
    for (int i = 0; i < partial_file_.size(); i++) {
        ScopedPtr<File> tmp_file(File::Open(
                this->partial_file_[i].filename(), O_RDWR | O_LARGEFILE |
                O_SYNC, 0));
        if (tmp_file.Get()) {
            partial_file_[i].Start(tmp_file.Get(), false);
                ContainerSuperblockData superblock;

                CHECK(tmp_file->ReadSizedMessage(0, &superblock,
                        kSuperBlockSize, true).valid(),
                    "Failed to read super block");
                DCHECK(log_data.partial_file_size() > i,
                    "Invalid partial file data");
                DCHECK(log_data.partial_file(i).has_uuid(), "Illegal log data");
                DCHECK(superblock.has_uuid(), "Illegal super block");

                    CHECK(log_data.partial_file(
                            i).uuid() == superblock.uuid(),
                        "Illegal container file uuid: " <<
                        "stored uuid " << superblock.uuid() <<
                        ", expected uuid " << log_data.file(i).uuid());

                Option<UUID> uuid = UUID::FromString(superblock.uuid());
                CHECK(
                    uuid.valid(), "Invalid uuid in super block: " <<
                    superblock.ShortDebugString());
                partial_file_[i].set_uuid(uuid.value());

            tmp_file.Release();
        } else {
            DEBUG("Create new partial container file " <<
                partial_file_[i].filename());
            CHECK(File::MakeParentDirectory(this->partial_file_[i].filename(),
                    start_context.dir_mode().mode()),
                "Failed to check parent directories");
            ScopedPtr<File> tmp_file(File::Open(
                    this->partial_file_[i].filename(), O_CREAT | O_RDWR | O_LARGEFILE |
                    O_SYNC, 0));
            CHECK(tmp_file.Get(), "Failed to open partial file: " << partial_file_[i].filename());

            CHECK(log_data.partial_file_size() <= i, "Illegal partial file data");
            partial_file_[i].set_uuid(UUID::Generate());
            ContainerSuperblockData superblock;
            superblock.set_uuid(partial_file_[i].uuid().ToString());

            CHECK(tmp_file.Get()->WriteSizedMessage(0, superblock, kSuperBlockSize, true) > 0,
              "Failed to write superblock: " << superblock.DebugString());
            CHECK(chmod(this->partial_file_[i].filename().c_str(),
                    start_context.file_mode().mode(
                        )) == 0,
                "Failed to change file permissions: " <<
                this->partial_file_[i].filename());
            if (start_context.file_mode().gid() != -1) {
                CHECK(chown(this->partial_file_[i].filename().c_str(), -1,
                        start_context.file_mode().gid()) == 0,
                    "Failed to change file group: " << this->partial_file_[i].filename());
            }

            partial_file_[i].Start(tmp_file.Get(), true);
            tmp_file.Release();
        }
    }
    partial_file_offset_.resize(partial_file_.size());
    for (int i = 0; i < partial_file_offset_.size(); i++) {
      partial_file_offset_[i] = kSuperBlockSize;
    }
    return true;
}

bool ContainerStorage::OpenContainerFiles(const ContainerLogfileData& log_data,
    const StartContext& start_context) {
    ...
    // b) file existed and was not stored in the log data. This happened on old systems that to not
    // have
    ...
    for (int i = 0; i < this->file_.size(); i++) {
        ScopedPtr<File> tmp_file(File::Open(
                this->file_[i].filename(), O_RDWR | O_LARGEFILE |
                O_SYNC, 0));
        if (tmp_file.Get()) {
            ...
            if (file_[i].file_size() == 0) {
                ...
                // there are two possibilities: a) log info entry exists => done b) backup path =>
                // assume default file size
                if (log_data.file_size() <= i ||
                    !log_data.file(i).has_file_size()) {
                ...
                if (log_data.file_size() > i &&
                    log_data.file(i).has_file_size()) {
                    CHECK(file_[i].file_size() == log_data.file(
                            i).file_size(),
                        "Illegal file size: " << file_[i].filename() <<
            ...
            TRACE(
                "Assign " << file_[i].file_size() << " to new file " <<
                file_[i].filename() << ", size to assign " << size_to_assign);
            ...
            if (log_data.file_size() > i) {
                ...
                CHECK(tmp_file->ReadSizedMessage(0, &superblock,
                        kSuperBlockSize, true).valid(),
                    "Failed to read super block");
                DCHECK(log_data.file(i).has_uuid(), "Illegal log data");
                DCHECK(superblock.has_uuid(), "Illegal super block");
            ...
            if (preallocate_) {
                ...
                CHECK(
                    fs.valid(), "Failed to get file size: " <<
                    file_[i].filename());
                uint64_t total_file_size = fs.value();
                total_file_size -= kSuperBlockSize;
                CHECK(
                    total_file_size == file_[i].file_size(), "Illegal file size: " <<
                    file_[i].filename() <<
    ...
    for (int i = 0; i < this->file_.size(); i++) {
        if (!file_[i].file()) {
            if (file_[i].file_size() > 0) {
                DEBUG(
                    "File " << file_[i].filename() << " has explicit size " <<
                    file_[i].file_size());
                size_to_assign -= file_[i].file_size();
            }else {
    ...
    DEBUG(
        "Size to assign: " << size_to_assign << ", files to assign to " <<
        file_count_no_explicit_fs);
    ...
    for (int i = 0; i < file_.size(); i++) {
        if (!file_[i].file()) {
            ...
            CHECK(
                log_data.file_size() <= i,
                "Error opening storage file: filename " <<
                this->file_[i].filename() <<
            ...
            if (file_size == 0) {
                int64_t default_size_per_file = size_to_assign /
                                                file_count_no_explicit_fs;
                CHECK(default_size_per_file > 0,
                    "Illegal container config: size to assign " <<
                    FormatStorageUnit(size_to_assign) <<
                ...
                DEBUG(
                    "Assign " << file_size << " to new file " <<
                    file_[i].filename());
            }
            // validate file size
            CHECK(file_size % container_size_ == 0,
                "Size per file is not aligned with container size: " <<
                "filename " << file_[i].filename() <<
                ", size " << file_size <<
                ", container size " << container_size_);
            uint64_t container_per_file = file_size / container_size_;
            CHECK(container_per_file % 8 == 0,
                "Containers per file must be devidable by 8: " <<
                "containers in file " << container_per_file <<
                ", file " << file_[i].filename() <<
                ", container file size " << file_size <<
                ", total size " << size_);

            CHECK(File::MakeParentDirectory(this->file_[i].filename(),
                    start_context.dir_mode().mode()),
                "Failed to check parent directories");
            ...
            ScopedPtr<File> format_file(File::Open(
                    this->file_[i].filename(), O_RDWR |
                    O_LARGEFILE | O_CREAT,
                    S_IRUSR |
                    S_IWUSR | S_IRGRP));
            CHECK(
                format_file.Get(), "Failed to open file for formatting: " <<
                this->file_[i].filename());
            CHECK(Format(this->file_[i],
                    format_file.Get()), "Failed to format " <<
                this->file_[i].filename());
            delete format_file.Release();

            ScopedPtr<File> tmp_file(File::Open(
                    this->file_[i].filename(), O_RDWR |
                    O_LARGEFILE | O_SYNC, 0));
            CHECK(
                tmp_file.Get(), "Failed to open container file " <<
                file_[i].filename());
            CHECK(chmod(this->file_[i].filename().c_str(),
                    start_context.file_mode().mode(
                        )) == 0,
                "Failed to change file permissions: " <<
                this->file_[i].filename());
            if (start_context.file_mode().gid() != -1) {
                CHECK(chown(this->file_[i].filename().c_str(), -1,
                        start_context.file_mode().gid()) == 0,
    ...
    CHECK(size_to_assign == 0,
        "Illegal container configuration: total size " <<
        FormatStorageUnit(size_) <<
        ", not assigned size " << FormatStorageUnit(size_to_assign));
    return true;
}

bool ContainerStorage::Start(const StartContext& start_context,
    DedupSystem* system)
{
    ...
    CHECK(this->container_size_ > 0,
        "Container size invalid: container size " << this->container_size_);
    ...
    CHECK(
        !(info_lookup == LOOKUP_NOT_FOUND && !start_context.create()),
    ...
    if (info_lookup == LOOKUP_FOUND) {
        CHECK(this->container_size_ == log_data.container_size(),
            "Container size mismatch (logged size " << log_data.container_size()
                                                    << ", configured size " <<
            this->container_size_ << ")");
        this->last_given_container_id_ = log_data.last_given_container_id();
    }

    if(!OpenContainerFiles(log_data, start_context)) {
      return false;
    }

    if(!OpenPartialContainerFiles(log_data, start_context)) {
      return false;
    }
    ...
    // when the state, e.g. the last given container id is incorrect here because of a dirty
    // shutdown
    ...
    CHECK(this->meta_data_index_->Start(
            start_context), "Container index start failed");
    ...
    if (this->idle_detector_) {
        CHECK(this->idle_detector_->RegisterIdleConsumer("container-storage",
                this),
            "Cannot register container storage as idle tick consumer");
    }

    CHECK(this->log_->RegisterConsumer("container-storage",
            this),
        "Cannot register container storage as log consumer");
    ...
    CHECK(this->allocator_->Start(start_context,
            this), "Cannot start allocator");
    ...
    if (!start_context.dirty() && !start_context_.readonly()) {
        ...
        pair<lookup_result,
            ContainerStorageAddressData> result = this->LookupContainerAddress(
            id,
            NULL,
            false);
        CHECK(result.first != LOOKUP_ERROR,
            "Meta data lookup failed: container " << id);
        if (result.first == LOOKUP_FOUND) {
            ERROR(
                "Last given container id " << id <<
                " already stored: Illegal last given container id");
            return false;
        }
    }else {
        // it is not valid to check the validity of the container id here as it
        // might be corrected by the log replay
        // or the system is readonly and we don't care because there is no way that the container is
        // overwritten.
    }
    INFO(
        "Started container storage (startup time: " <<
        startup_timer.GetTime() <<
        "ms" <<
        ", last given container id: " << this->last_given_container_id_ << ")");
...
bool ContainerStorage::RecoverContainerFromPartials() {

    INFO("Recover container");
    map<uint64_t, Container*> recovered_container_map;
    map<uint64_t, ContainerStorageAddressData> recovered_address_map;

    for (int i = 0; i < partial_file_.size(); i++) {
        File* file = partial_file_[i].file();
        DCHECK(file, "File not set");
        uint64_t offset = kSuperBlockSize;
        Option<int64_t> file_size = file->GetSize();
        CHECK(file_size.valid(), "Failed to get file size");
        ContainerPartialData partial_data;

        while (offset < file_size.value()) {
            Option<size_t> s = file->ReadSizedMessage(offset,&partial_data, container_size_, true);
            CHECK(s.valid(), "Failed to read partial data: " << offset);
            TRACE("Read partial file to recover: offset " << offset);

            if (partial_data.has_address_info()) {
                DEBUG("Recover container address: " <<
                    partial_data.address_info().ShortDebugString());
                recovered_address_map[partial_data.address_info().container_id()] =
                  partial_data.address_info().address();
            }

            for (int j = 0; j < partial_data.partial_item_size(); j++) {
                const ContainerPartialItemData& item_data(partial_data.partial_item(j));

                 enum storage_commit_state scs = IsCommitted(item_data.container_id());
                  CHECK(scs != STORAGE_ADDRESS_ERROR, "Failed to check commit state");
                if (scs != STORAGE_ADDRESS_COMMITED) {
                    INFO("Need to restore: " <<
                        "fp " << ToHexString(item_data.fp().data(), item_data.fp().size()) <<
                        ", container id " << item_data.container_id() <<
                        ", data size " << item_data.data().size());

                Container* container = recovered_container_map[item_data.container_id()];
                if (container == NULL) {
                    container = new Container(item_data.container_id(), container_size_, false);
                    recovered_container_map[item_data.container_id()] = container;
                }

                CHECK(container->AddItem(item_data.fp().data(), item_data.fp().size(),
                        item_data.data().data(), item_data.data().size(),
                        item_data.indexed(),
                        compression_),
                    "Failed to add item to recover container: " <<
                    "partial container " << container->DebugString());
                }
            }
            offset += s.value();
        }
    ...
    INFO("Need to recover: " << recovered_container_map.size() << " containers");

    map<uint64_t, Container*>::iterator k;
    for (k = recovered_container_map.begin(); k != recovered_container_map.end(); k++) {
        CHECK(recovered_address_map.find(k->first) != recovered_address_map.end(),
            "Failed to find recovery address: container id " << k->first);
        ContainerStorageAddressData address_data = recovered_address_map[k->first];
        Container* container = k->second;

        INFO("Recover " << container->DebugString());
        CHECK(CommitContainer(container, address_data),
    ...
    for (int i = 0; i < partial_file_.size(); i++) {
        File* file = partial_file_[i].file();
        DCHECK(file, "File not set");
        CHECK(file->Truncate(kSuperBlockSize),
            "Failed to truncate a file");
        partial_file_offset_[i] = kSuperBlockSize;
    }
...
bool ContainerStorage::Run()
{
    ...
    CHECK(partial_commit_cleanup_thread_.Start(),
        "Failed to run partial commit cleanup thread");
    if (allocator_) {
...
ContainerStorage::~ContainerStorage() {
    ...
    for (vector<Container*>::iterator i = this->write_cache_.GetCache().begin();
         i != this->write_cache_.GetCache().end();
         i++) {
        Container* write_container = *i;
        if (write_container &&
            write_container->primary_id() != 0 &&
            write_container->primary_id() !=
            Storage::ILLEGAL_STORAGE_ADDRESS &&
            write_container->item_count() > 0) {
            WARNING(
                "Container " << write_container->primary_id() <<
                " not committed during shutdown");
        }
    }
    ...
    for (i = 0; i < this->partial_file_.size(); i++) {
        if (this->partial_file_[i].file()) {
            delete partial_file_[i].file();
            partial_file_[i].clear_file();
        }
    }
    partial_file_.clear();
    ...
    if (this->idle_detector_ &&
        this->idle_detector_->IsRegistered("container-storage").value()) {
        if (!this->idle_detector_->UnregisterIdleConsumer("container-storage")) {
            WARNING(
                "Failed to unregister container storage (idle tick consumer");
        }
    }

    if ( this->partial_commit_cleanup_thread_.IsJoinable()) {
        if (!this->partial_commit_cleanup_thread_.Join(
                NULL)) {
            WARNING("Thread returned false");
        }
    }
    chunk_index_ = NULL;
}
...
bool ContainerStorage::Stop(const dedupv1::StopContext& stop_context)
{
    ...
    DEBUG("Stopping container storage: " << state_);
    if (state_ == STARTING || state_ == STARTED || state_ == RUNNING) {
        INFO("Stopping container storage");
        state_ = STOPPING;
    }

    DEBUG("Stop cleanup committer");
    if (this->partial_commit_cleanup_thread_.IsJoinable()) {
        bool result = false;
        CHECK(this->partial_commit_cleanup_thread_.Join(
                &result), "Thread returned false");
        if (!result) {
            WARNING("Thread stopped with an error");
            failed = true;
        }
    }
    ...
    for (vector<Container*>::iterator i = this->write_cache_.GetCache().begin();
        ...
        if (write_container &&
            ...
            if (address_map_.find(write_container->primary_id()) ==
                address_map_.end()) {
                ERROR(
                    "Failed to get pre-assigned address for container: " <<
                    write_container->DebugString());
                failed = true;
            }else if (!this->CommitContainer(write_container,
                          address_map_[write_container->primary_id()])) {
                ERROR(
                    "Failed to commit container: " <<
                    write_container->DebugString());
                failed = true;
...
bool ContainerStorage::DumpMetaInfo()
{
    ...
    for (int i = 0; i < partial_file_.size(); i++) {
        ContainerPartialFileData* partial_file_data = data.add_partial_file();
        partial_file_data->set_filename(partial_file_[i].filename());
        partial_file_data->set_uuid(partial_file_[i].uuid().ToString());
    }

    CHECK(info_store_->PersistInfo("container-storage",
            data), "Failed to persist container storage info: " <<
...
lookup_result ContainerStorage::ReadMetaInfo(ContainerLogfileData* log_data)
{
    ...
    CHECK_RETURN(lr != LOOKUP_ERROR,
        LOOKUP_ERROR,
        "Failed to read container storage info");
...
bool ContainerStorage::Flush(dedupv1::base::ErrorContext* ec)
{
    ...
    for (uint32_t i = 0; i < this->write_cache_.GetSize(); i++) {
        ...
        CHECK(this->write_cache_.GetWriteCacheContainerByIndex(i,
                &check_write_container,
                &write_cache_lock),
        ...
        CHECK(
            scoped_write_container_lock.AcquireWriteLock(),
            "Failed to acquire write cache lock");

        if (check_write_container->primary_id() &&
            check_write_container->primary_id() !=
            Storage::ILLEGAL_STORAGE_ADDRESS) {
            ProfileTimer commit_timer(this->stats_.pre_commit_time_);

            CHECK(address_map_lock_.AcquireLock(), "Failed to acquire lock");
            std::map<uint64_t, ContainerStorageAddressData>::iterator it =
                address_map_.find(check_write_container->primary_id());
            if (it == address_map_.end()) {
                ERROR(
                    "Failed to get pre-assigned address for container: " <<
                    check_write_container->DebugString());
                failed = true;
            }else {
                if (!this->CommitContainer(check_write_container,
                        it->second)) {
                    ERROR(
                        "Failed to commit container: " <<
                        check_write_container->DebugString());
            ...
            CHECK(address_map_condition_.Broadcast(),
                "Failed to broadcast address map change");
            CHECK(address_map_lock_.ReleaseLock(), "Failed to release lock");
            // check write container has no a ILLEGAL_STORAGE_ADDRESS
            // The container gets a new id when a chunk is added
        }
        CHECK(
            scoped_write_container_lock.ReleaseLock(),
            "Cannot unlock write container");
    }
...
alloc_result ContainerStorage::GetNewContainerId(Container* container)
{
    ...
    alloc_result ar = this->allocator_->OnNewContainer(*container,
        true,
        &address);
    ...
    uint64_t last_given_container_id =
        this->last_given_container_id_.fetch_and_increment() + 1;
    container->Reuse(last_given_container_id);

    CHECK_RETURN(
        address_map_lock_.AcquireLock(), ALLOC_ERROR,
        "Failed to acquire lock");
    address_map_[container->primary_id()] = address;
    CHECK_RETURN(
        address_map_condition_.Broadcast(), ALLOC_ERROR,
        "Failed to broadcast change");
    CHECK_RETURN(
        address_map_lock_.ReleaseLock(), ALLOC_ERROR,
    ...
    DEBUG("Write container address to partial file: " <<
        "container id " << last_given_container_id <<
        ", address " << address.ShortDebugString());
    ContainerPartialData partial_data;
    partial_data.mutable_address_info()->set_container_id(last_given_container_id);
    partial_data.mutable_address_info()->mutable_address()->CopyFrom(address);
    CHECK_RETURN(WriteContainerPartial(partial_data, NO_EC), ALLOC_ERROR,
        "Failed to write container parital data");

    // TODO (dmeister) Check if we can remove the Container Open event
    Option<int64_t> commit_result = this->log_->CommitEvent(
            dedupv1::log::
            EVENT_TYPE_CONTAINER_OPEN,
            &event_data,
            NO_EC);
    CHECK_RETURN(commit_result.valid(),
        ALLOC_ERROR, "Failed to commit container event: " <<
        event_data.ShortDebugString());

    CHECK_RETURN(PostContainerEvent(*container,
          EVENT_TYPE_CONTAINER_OPEN,
          &event_data,
          commit_result.value()),
        ALLOC_ERROR,
        "Failed to handle container event post-processing");
    DEBUG(
        "Open container: container id " << last_given_container_id <<
        ", address " <<
        DebugString(event_data.address()));
    return ALLOC_OK;
}
...
bool ContainerStorage::PrepareCommit(Container* container)
{
    ...
    ScopedLock scoped_lock(&address_map_lock_);
    CHECK(scoped_lock.AcquireLock(), "Failed to acquire lock");
    std::map<uint64_t, ContainerStorageAddressData>::iterator it =
        address_map_.find(container->primary_id());
    if (it == address_map_.end()) {
        ERROR(
            "Failed to get pre-assigned address for container: " <<
            container->DebugString());
        ...
        TRACE("Hand over container: container " << container->DebugString() <<
            ", address " << DebugString(it->second));
        ContainerStorageAddressData address_data = it->second;

        // This call might block, if old container is not yet written to disk
        dedupv1::base::timed_bool r = background_committer_.Handover(container,
            address_data);
        CHECK(r != TIMED_FALSE,
            "Failed to handover container: " << container->DebugString());
        CHECK(r != TIMED_TIMEOUT,
            "Timeout while handing over container: " << container->DebugString());

        address_map_.erase(it);
        CHECK(address_map_condition_.Broadcast(), "Failed to broadcast change");
        CHECK(scoped_lock.ReleaseLock(), "Failed to release lock");

        // We never have a container lock on a container that is not committed
        CacheEntry cache_entry;
        lookup_result lr = cache_.GetCache(
            container->primary_id(), &cache_entry);
        CHECK(
            lr != LOOKUP_ERROR,
            "Failed to check for cache line: container " <<
            container->DebugString());
        if (lr == LOOKUP_NOT_FOUND && cache_entry.is_set()) {
            CHECK(this->cache_.CopyToReadCache(*container,
                    &cache_entry),
                "Failed to copy container to read cache: " <<
                container->DebugString());
        } else{
...
bool ContainerStorage::PrepareWriteContainer(Container* write_container,
    const StorageRequest& request,
    ErrorContext* ec)
{
    bool new_container_id = !Storage::IsValidAddress(write_container->primary_id());
    if (!new_container_id &&
        write_container->IsFull(request.key_size(), request.data_size())) {
        // we do not need to reset the container cache timeout, because it has
        // been reseted a few moments ago (GetNextWriteCacheContainer)
        CHECK(PrepareCommit(write_container),
            "Failed to prepare commit: " << write_container->DebugString());

        new_container_id = true;
    }
    ...
    DCHECK(
        ...
        Storage::EMPTY_DATA_STORAGE_ADDRESS &&
        container_id != Storage::ILLEGAL_STORAGE_ADDRESS,
        "Illegal container id: " << write_container->primary_id());
    return true;
}

bool ContainerStorage::WriteNew(list<StorageRequest>* requests,
    ErrorContext* ec)
{
    ProfileTimer timer(stats_.total_write_time_);

    CHECK(state_ == RUNNING,
        "Illegal state to write new data: " << state_);
    CHECK(
        !start_context_.readonly(),
        "Container storage is in readonly mode");

    // Select write container
    Container* write_container = NULL;
    ReadWriteLock* write_container_lock = NULL;
    CHECK(write_cache_.GetNextWriteCacheContainer(&
            write_container,
            &write_container_lock),
    ...
    ContainerPartialData partial_data;
    list<StorageRequest>::iterator i;
    for (i = requests->begin(); i != requests->end(); i++) {
        StorageRequest& request(*i);

        TRACE("Write storage request: " << request.DebugString());

        if (!PrepareWriteContainer(write_container, request, ec)) {
            return false;
        }
        uint64_t container_id = write_container->primary_id();
        request.set_address(container_id);

        ProfileTimer add_timer(stats_.add_time_);
        CHECK(write_container->AddItem(request.key(),
                ...
                compression_),
            "Cannot add item: fp " <<
            Fingerprinter::DebugString(request.key(), request.key_size()) <<
            ", data size " << request.data_size() <<
            ", write container " << write_container->DebugString());
        // Here I assume that a partial container is NEVER full.

        ContainerPartialItemData* partial_item =
            partial_data.add_partial_item();
        partial_item->set_fp(request.key(), request.key_size());
        partial_item->set_container_id(write_container->primary_id());
        partial_item->set_indexed(request.is_indexed());
        partial_item->set_data(request.data(), request.data_size());
        add_timer.stop();

        TRACE("Finished storage request: " << request.DebugString());
    }

    ProfileTimer partial_timer(stats_.partial_log_time_);
    CHECK(WriteContainerPartial(partial_data, ec),
        "Failed to write partial event data");
    partial_timer.stop();

    CHECK(
        scoped_write_container_lock.ReleaseLock(),
        "Failed to release write container lock: " <<
        scoped_write_container_lock.DebugString());
    return true;
}

bool ContainerStorage::PersistStatistics(std::string prefix,
    dedupv1::PersistStatistics* ps)
{
    ...
    CHECK(ps->Persist(prefix,
            data), "Failed to persist container storage stats");

    if (gc_) {
        CHECK(gc_->PersistStatistics(prefix + ".gc",
                ps), "Failed to persist gc");
    }
    CHECK(cache_.PersistStatistics(prefix + ".cache",
            ps), "Failed to persist cache");
    CHECK(write_cache_.PersistStatistics(prefix + ".write_cache",
            ps), "Failed to persist write cache");
    if (allocator_) {
        CHECK(allocator_->PersistStatistics(prefix + ".alloc",
                ps), "Failed to persist allocator");
    }
    return true;
}

bool ContainerStorage::RestoreStatistics(std::string prefix,
    dedupv1::PersistStatistics* ps)
{
    ContainerStorageStatsData data;
    CHECK(ps->Restore(prefix,
            &data), "Failed to restore container storage cache stats");
    ...
    if (gc_) {
        CHECK(gc_->RestoreStatistics(prefix + ".gc",
                ps), "Failed to restore gc");
    }
    CHECK(cache_.RestoreStatistics(prefix + ".cache",
            ps), "Failed to restore cache");
    CHECK(write_cache_.RestoreStatistics(prefix + ".write_cache",
            ps), "Failed to restore write cache");
    if (allocator_) {
        CHECK(allocator_->RestoreStatistics(prefix + ".alloc",
                ps), "Failed to restore allocator");
    }
    return true;
}
...
string ContainerStorage::PrintStatistics()
{
    ...
    sstr << "{";
    sstr << "\"gc\": " <<
    (this->gc_ ? this->gc_->PrintStatistics() : "null") << "," <<
    std::endl;
    sstr << "\"allocator\": " <<
    (this->allocator_ ? this->allocator_->PrintStatistics() : "null") << "," <<
    std::endl;
    sstr << "\"write cache\": " << this->write_cache_.PrintStatistics() <<
    "," <<
    std::endl;
    sstr << "\"read cache\": " << this->cache_.PrintStatistics() << "," <<
    std::endl;
    sstr << "\"data size\": " << allocated_storage_size << "," << std::endl;
    sstr << "\"allocated storage size\": " << allocated_storage_size << "," <<
    std::endl;
    sstr << "\"active storage size\": " << GetActiveStorageDataSize() << "," <<
    std::endl;

    sstr << "\"reads\": " << this->stats_.reads_ << "," << std::endl;
    sstr << "\"write cache hits\": " << this->stats_.write_cache_hit_ << "," <<
    std::endl;
    sstr << "\"committed container\": " << this->stats_.committed_container_ <<
    "," <<
    std::endl;
    sstr << "\"readed container\": " << this->stats_.readed_container_ <<
    "," <<
    std::endl;

    sstr << "\"moved container\": " << this->stats_.moved_container_ << "," <<
    std::endl;
    sstr << "\"merged container\": " << this->stats_.merged_container_ <<
    "," <<
    std::endl;
    sstr << "\"failed container\": " << this->stats_.failed_container_ <<
    "," <<
    std::endl;
    sstr << "\"deleted container\": " << this->stats_.deleted_container_ <<
    "" <<
    std::endl;
    sstr << "}";
    return sstr.str();
...
string ContainerStorage::PrintTrace()
{
    stringstream sstr;
    sstr << "{";
    sstr << this->background_committer_.PrintEmbeddedTrace() << "," <<
    std::endl;
    sstr << "\"gc\": " << (this->gc_ ? this->gc_->PrintTrace() : "null") <<
    "," <<
    std::endl;
    sstr << "\"allocator\": " <<
    (this->allocator_ ? this->allocator_->PrintTrace() : "null") << "," <<
    std::endl;
    sstr << "\"read cache\": " << this->cache_.PrintTrace() << "," << std::endl;
    sstr << "\"write cache\": " << this->write_cache_.PrintTrace() << "," <<
    std::endl;
    sstr << "\"meta data items\": " <<
    (this->meta_data_index_ ? ToString(this->meta_data_index_->GetItemCount())
     :
     "null") << "," << std::endl;
    sstr << "\"meta data size\": " <<
    (this->meta_data_index_ ? ToString(this->meta_data_index_->
         GetPersistentSize())
     : "null") << "," << std::endl;
    sstr << "\"partial container file truncate count\": " <<
    stats_.partial_container_file_truncate_count_ << "," << std::endl;
    sstr << "\"last given container id\": " <<
    GetLastGivenContainerId() << "" << std::endl;
    sstr << "}";
    return sstr.str();
}
...
string ContainerStorage::PrintLockStatistics()
{
    ...
    sstr << "{";
    sstr << "\"gc\": " <<
    (this->gc_ ? this->gc_->PrintLockStatistics() : "null") <<
    "," << std::endl;
    sstr << "\"allocator\": " <<
    (this->allocator_ ? this->allocator_->PrintLockStatistics() : "null") <<
    "," << std::endl;
    sstr << "\"read cache\": " << this->cache_.PrintLockStatistics() << "," <<
    std::endl;
    sstr << "\"write cache\": " << this->write_cache_.PrintLockStatistics() <<
    "," <<
    std::endl;
    sstr << "\"meta data\": " <<
    (this->meta_data_index_ ? this->meta_data_index_->PrintLockStatistics() :
     "null") << "," << std::endl;
    sstr << "\"file lock free\": " << this->stats_.file_lock_free_ << "," <<
    std::endl;
    sstr << "\"file lock busy\": " << this->stats_.file_lock_busy_ << "," <<
    std::endl;
    sstr << "\"handover lock free\": " << this->stats_.handover_lock_free_ <<
    "," <<
    std::endl;
    sstr << "\"handover lock busy\": " << this->stats_.handover_lock_busy_ <<
    "," <<
    std::endl;
    sstr << "\"container lock free\": " << this->stats_.container_lock_free_ <<
    "," <<
    std::endl;
    sstr << "\"container lock busy\": " << this->stats_.container_lock_busy_ <<
    std::endl;
    sstr << "}";
    return sstr.str();
}
...
string ContainerStorage::PrintProfile()
{
    stringstream sstr;
    sstr << "{";
    sstr << this->background_committer_.PrintEmbeddedProfile() << "," <<
    std::endl;
    sstr << "\"gc\": " << (this->gc_ ? this->gc_->PrintProfile() : "null") <<
    "," <<
    std::endl;
    sstr << "\"allocator\": " <<
    (this->allocator_ ? this->allocator_->PrintProfile() : "null") << "," <<
    std::endl;
    sstr << "\"meta data\": " <<
    (this->meta_data_index_ ? this->meta_data_index_->PrintProfile() : "null")
         <<
    "," << std::endl;
    sstr << "\"read container time\": " <<
    this->stats_.total_read_container_time_.GetSum() << "," << std::endl;
    sstr << "\"read cache\": " << this->cache_.PrintProfile() << "," <<
    std::endl;
    sstr << "\"write cache\": " << this->write_cache_.PrintProfile() << "," <<
    std::endl;
    sstr << "\"read time\": " << this->stats_.total_read_time_.GetSum() <<
    "," <<
    std::endl;
    sstr << "\"write time\": " << this->stats_.total_write_time_.GetSum() <<
    "," <<
    std::endl;
    sstr << "\"add time\": " << this->stats_.add_time_.GetSum() << "," <<
    std::endl;
    sstr << "\"partial log time\": " << stats_.partial_log_time_.GetSum() << "," <<
    std::endl;
    sstr << "\"pre commit time\": " <<
    this->stats_.pre_commit_time_.GetSum() << "," <<
    std::endl;
    sstr << "\"container write time\": " <<
    this->stats_.container_write_time_.GetSum() << "," << std::endl;
    sstr << "\"commit state check time\": " <<
    this->stats_.is_committed_time_.GetSum() << "," << std::endl;
    sstr << "\"file lock time\": " <<
    this->stats_.total_file_lock_time_.GetSum() <<
    "," << std::endl;
    sstr << "\"file load time\": " <<
    this->stats_.total_file_load_time_.GetSum() <<
    "," << std::endl;
    sstr << "\"replay time\": " << this->stats_.replay_time_.GetSum() << "," <<
    std::endl;
    sstr << "\"average container load latency\": " <<
    this->stats_.total_file_load_time_.GetAverage() << "," << std::endl;
    sstr << "\"delete time\": " << this->stats_.total_delete_time_.GetSum() <<
    std::endl;
    sstr << "}";
    return sstr.str();
}
    ...
    ErrorContext* ec)
{
    ...
    // we know for sure that the primary id is the same as no one else was allowed to do changes on
    // the container

    ReadWriteLock* container_lock = NULL;
    std::pair<dedupv1::base::lookup_result, ContainerStorageAddressData> address2;
    address2 = LookupContainerAddress(container_id,
        &container_lock,
        true);
    ...
    if (address2.second.has_primary_id() && address2.second.primary_id() !=
        primary_id) {
    ...
    Container container(container_id, container_size(), false);

    // we have to force a read from disk to ensure that we use the latest version
    // if would not remove the container from the cache and use a cache-aware container read method
    // (ReadContainerWithCache)
    // the container would be stored on disk without the deleted item, but the container with the
    // item would remain in the
    // read cache. This would lead to race conditions

    if (cache_entry->is_set()) {
        CHECK(cache_.RemoveFromReadCache(primary_id,
                cache_entry),
            "Failed to remove container from read cache: container " <<
            primary_id);
    }
    // if we held the cache entry lock before, we released it now

    // we have the container lock already
    lookup_result read_result = ReadContainerFromDiskLocked(&container, address);
    CHECK(
        read_result != LOOKUP_ERROR,
        "Read of container failed: id " << container_id << ", primary id " << primary_id <<
        ", reason: index lookup error");
    CHECK(
        read_result != LOOKUP_NOT_FOUND,
        "Read of container failed: id " << container_id << ", primary id " << primary_id <<
        ", reason: container not found");

    // found
    CHECK(container.HasId(
            container_id),
        "Wrong active container: " << container.DebugString() <<
        ", address " <<
        container_id);
    ...
    for (list<bytestring>::const_iterator i = key_list.begin();
         i != key_list.end();
         i++) {
        const byte* key = reinterpret_cast<const byte*>(i->data());
        size_t key_size = i->size();
        TRACE("Delete item " << Fingerprinter::DebugString(key,
                key_size) <<
            " from container " << container.DebugString() <<
            " (read cache/disk)");
        CHECK(container.DeleteItem(key, key_size),
            "Failed to delete container item: " <<
            Fingerprinter::DebugString(key,
                key_size) << " from container " << container_id);
    }
    ContainerStorageAddressData new_container_address;
    ContainerStorageAddressData old_container_address = address;

    if (IsCommitted(container.primary_id()) !=
        STORAGE_ADDRESS_COMMITED) {
        TRACE(
            "Container " << container.primary_id() <<
            " is not-open, but uncommitted. Check for bg committer");
        // not open and not committed, means either an serious error or a bg committer
        Option<bool> b =
            background_committer_.
            IsCurrentlyProcessedContainerId(
                container.primary_id());
        DCHECK(
            b.valid(),
            "Failed to check is container id is currently processed");

        if (b.value()) {
            // we have luck
            enum storage_commit_state scs = IsCommitted(
                container.primary_id());
            size_t i = 0;
            while (scs == STORAGE_ADDRESS_NOT_COMMITED && i <
                   (ContainerStorageBackgroundCommitter::kMaxThreads + 1)) {
                CHECK(
                    background_committer_.
                    CommitFinishedConditionWaitTimeout(
                        10, dedupv1::base::timeunit::SECONDS) == TIMED_TRUE,
                    "Failed to wait for container commit");
                scs = IsCommitted(container.primary_id());
                i++;
            }
            CHECK(scs == STORAGE_ADDRESS_COMMITED,
                "Container " << container.primary_id() << " still uncommitted");
        }else {
            ERROR(
                "Cannot find non-open, but uncommitted container " <<
                container.primary_id());
            return false;
        }
    }

    CHECK(allocator_->OnNewContainer(container,
            false,
            &new_container_address), "Failed to get new container address");
    ...
    INFO("Moved container " << container.DebugString() <<
        ", old address " << ContainerStorage::DebugString(
            old_container_address) <<
    ...
    // Commit data only when on critical lock are hold, due to DIRECT processing
    Option<int64_t> commit_result = log_->CommitEvent(EVENT_TYPE_CONTAINER_MOVED,
        &event_data, NO_EC);
    CHECK(commit_result.valid(),
        "Failed to commit container move");

    CHECK(PostContainerEvent(container,
          EVENT_TYPE_CONTAINER_MOVED,
          &event_data,
          commit_result.value()),
        "Failed to handle commit event");
...
bool ContainerStorage::WriteContainerPartial(
    const ContainerPartialData& partial_data,
    ErrorContext* ec) {

    int partial_write_index = 0;
    bool locked = false;
    for (int i = 0; i < partial_file_.size(); i++) {
        partial_write_index = (partial_write_count_++) % partial_file_.size();
        ContainerFile& container_file(partial_file_[partial_write_index]);
        DCHECK(container_file.file(), "Partial container file not set");
        CHECK(container_file.lock()->TryAcquireLock(&locked),
            "Failed to acquire partial file lock");
        if (locked) {
            break;
        }
    }
    ...
    DCHECK(container_file.file(), "Partial container file not set");
    if (!locked) {
        CHECK(container_file.lock()->AcquireLock(),
            "Failed to acquire partial file lock");
    }

    bool failed = false;
    int64_t offset = partial_file_offset_[partial_write_index];
    DEBUG("Write container partial: " <<
        "file " << container_file.file()->path() <<
        ", offset " << offset);
    ssize_t r = container_file.file()->WriteSizedMessage(offset,
        partial_data, container_size_, true);
    if (r < 0) {
        ERROR("Failed to write partial container data: " <<
            container_file.filename() <<
            ", offset " << offset <<
            ", partial data size " << partial_data.ByteSize() <<
            ", result " << r);
        failed = true;
    } else {
        partial_file_offset_[partial_write_index] += r;
    }
    CHECK(container_file.lock()->ReleaseLock(),
        "Failed to release partial file lock");
    return !failed;
}
    ...
    ErrorContext* ec)
{
    ...
    CHECK(state_ == RUNNING || state_ == STARTED,
        "Illegal state to delete data: " << state_);

    TRACE("Delete storage entry: address " << container_id);

    enum lookup_result r = write_cache_.GetWriteCacheContainer(container_id,
        &write_container,
        &write_container_lock,
        true);
    ...
    while (!has_access) {
        address = LookupContainerAddress(container_id,
            &container_lock,
            true);
        CHECK(address.first != LOOKUP_ERROR,
            "Failed to get address of container " << container_id);
        CHECK(address.first != LOOKUP_NOT_FOUND,
            "Failed to get address of container " << container_id <<
            ", reason: container not found");
    ...
    CacheEntry cache_entry;
    lookup_result cache_lookup_result = GetReadCache()->CheckCache(primary_id,
        &cached_container,
        true,
        true,
        &cache_entry);
    ...
    if (cache_lookup_result == LOOKUP_ERROR) {
        ...
        delete_result = DoDelete(container_id,
            primary_id,
            address.second,
            key_list,
            &cache_entry,
...
Option<uint32_t> ContainerStorage::ReadInContainer(const Container &container,
    ...
    if (item == NULL) {
        ...
        for (i = container.items().begin(); i != container.items().end();
             i++) {
        ...
        WARNING("Key not found in container: " <<
            ...
            ", key " <<
            Fingerprinter::DebugString(static_cast<const byte*>(key), key_size));
        if (found) {
            *found = false;
        }
    ...
    CHECK(
        !item->is_deleted(),  "Found a deleted item: " <<
        item->DebugString());
    CHECK(
        offset + size <= item->raw_size(),
        "Data length error: " <<
        "container " << container.DebugString() <<
        ...
        ", data size " << size);
    if (data) {
        CHECK(container.CopyRawData(item,
                data,
                offset,
                size),
            "Cannot copy data: " <<
...
Option<uint32_t> ContainerStorage::ReadChunk(uint64_t address, const void* key,
    ...
    ErrorContext* ec)
{
    ...
    CHECK(state_ == RUNNING ||
        state_ == STARTED, "Illegal state to read data: " << state_);
    DCHECK(key, "Key NULL");

    Container* write_container = NULL;
    ReadWriteLock* write_cache_lock = NULL;
    lookup_result r = write_cache_.GetWriteCacheContainer(address,
        &write_container,
        &write_cache_lock,
        false);
    CHECK(r != LOOKUP_ERROR, "Failed to access write cache");
    if (r == LOOKUP_FOUND) {
        ...
        stats_.write_cache_hit_++;
        TRACE("Read item " << Fingerprinter::DebugString((byte *) key,
                key_size) << " from container " << address << " (write cache)");

        bool found = false;
        Option<uint32_t> r = ReadInContainer(*write_container,
            key,
            key_size,
            data,
            offset,
    ...
    CHECK(read_result != LOOKUP_ERROR,
        "Read of container " << address << " failed: Cache check failed");
    if (read_result == LOOKUP_FOUND) {
        ...
        CHECK(cache_container->HasId(
                address),
            "Wrong active container: " << cache_container->DebugString() <<
            ", address " << address);

        TRACE("Read item " << Fingerprinter::DebugString((byte *) key,
                key_size) << " from container " <<
            cache_container->DebugString(
                ) <<
            " (read cache)");

        // We hold the cache entry lock, we can now also allocate the container lock
        bool found = false;
        Option<uint32_t> r = ReadInContainer(*cache_container,
            key,
            key_size,
            data,
            offset,
    ...
    read_result = ReadContainerFromDisk(&read_container); // without cache
    if (read_result == LOOKUP_ERROR) {
        ...
        if (cache_entry.is_set()) {
            if (!cache_.ReleaseCacheline(address, &cache_entry)) {
                WARNING(
                    "Failed to release cache line: container id " << address <<
                    ", cache line " << cache_entry.DebugString());
            }
        }
    ...
    // found
    CHECK(read_container.HasId(address),
        "Wrong active container: " << read_container.DebugString() << ", address " << address);
    bool found = false;
...
storage_commit_state ContainerStorage::IsCommitted(uint64_t address)
{
    ...
    Option<bool> is_processed =
        this->background_committer_.IsCurrentlyProcessedContainerId(address);
    while (is_processed.valid() && is_processed.value()) {
        TRACE(
            "Wait until currently processed container is committed: container id "
            <<
            address);
        dedupv1::base::timed_bool b =
            this->background_committer_.CommitFinishedConditionWaitTimeout(
                1, dedupv1::base::timeunit::SECONDS);
        CHECK_RETURN(b != TIMED_FALSE,
    ...
    lookup_result r = this->meta_data_index_->Lookup(&address,
        sizeof(uint64_t),
        NULL);
    CHECK_RETURN(r != LOOKUP_ERROR,
        STORAGE_ADDRESS_ERROR,
        "Meta data index lookup error for container id " << address);
    ...
    if (commit_state == STORAGE_ADDRESS_COMMITED) {
        TRACE(
            "Checked commit state: container id " << address << ", result " <<
            commit_state << ", source index");
    }
    return commit_state;
}
    ...
    const dedupv1::log::LogReplayContext & context)
{
    ...
    if (event_type == log::EVENT_TYPE_CONTAINER_OPEN &&
        context.replay_mode() ==
        EVENT_REPLAY_MODE_REPLAY_BG) {
        ContainerOpenedEventData event_data =
            event_value.container_opened_event();
    ...
    }else if (event_type == EVENT_TYPE_CONTAINER_OPEN &&
              context.replay_mode() ==
              EVENT_REPLAY_MODE_DIRTY_START) {
        ContainerOpenedEventData event_data =
            event_value.container_opened_event();
        container_id = event_data.container_id();
        ...
        if (container_id > last_given_container_id_) {
            if (container_id > last_given_container_id_) {
                DEBUG(
                    "Recover last given container id " << container_id <<
                    " from log");
              ...
              EVENT_REPLAY_MODE_DIRTY_START) {
        ContainerCommittedEventData event_data =
            event_value.container_committed_event();
        ...
        lookup_result lr = meta_data_index_->Lookup(&container_id,
            sizeof(container_id),
            &address_data);
        CHECK(
            lr != LOOKUP_ERROR,
            "Failed to lookup meta data for container: container id " <<
            container_id);

        bool update =
            (lr == LOOKUP_NOT_FOUND || address_data.log_id() <
             context.log_id());
        if (update) {
            // Update meta data
            ...
            DEBUG(
                "Update meta data for container: container id " <<
                container_id <<
                ", address " << updated_address.ShortDebugString() <<
                ", old address data " << address_data.ShortDebugString());

            DCHECK(IsValidAddressData(
                    event_data.address()), "Invalid address data: " <<
                event_data.ShortDebugString());
            CHECK(meta_data_index_->Put(&container_id,
                    sizeof(container_id),
                    updated_address),
                "Failed to update meta data index data: container id " <<
                container_id <<
                ", address " << updated_address.ShortDebugString());
            ...
            TRACE(
                "Meta data already up-to-date: container id " <<
                container_id <<
        ...
        if (update) {
            // Update meta data
            ContainerStorageAddressData updated_address =
                event_data.new_address();
            updated_address.set_log_id(context.log_id());

            DCHECK(IsValidAddressData(
                    updated_address), "Invalid address data: " <<
                updated_address.ShortDebugString());
            CHECK(this->meta_data_index_->Put(&container_id,
                    sizeof(container_id),
                    updated_address),
                "Meta data update failed: " << container_id <<
        ...
        ContainerMergedEventData event_data =
            event_value.container_merged_event();
        uint64_t container_id = event_data.new_primary_id();
        ...
        for (int i = 0; i < event_data.new_secondary_id_size(); i++) {
            ...
            lookup_result lr = meta_data_index_->Lookup(&id,
                sizeof(id),
                &address_data);
            CHECK(lr != LOOKUP_ERROR,
                "Failed to lookup meta data for container: container id " << id);
            bool update =
                (lr == LOOKUP_NOT_FOUND || address_data.log_id() <
                 context.log_id());
        ...
        for (int i = 0; i < event_data.unused_ids_size(); i++) {
            ...
            if (lr == LOOKUP_FOUND) {
                ...
                CHECK(this->meta_data_index_->Delete(&id,
                        sizeof(id)),
                    "Cannot delete unused container address: container id " <<
                    id);
            }
        }
    }else if (event_type == EVENT_TYPE_CONTAINER_DELETED &&
        ...
        ContainerDeletedEventData event_data =
            event_value.container_deleted_event();
        uint64_t container_id = event_data.container_id();

        DEBUG("Log replay: " << Log::GetEventTypeName(
                event_type) << ", event " << event_data.ShortDebugString());
...
bool ContainerStorage::LogReplayStateChange(const dedupv1::log::LogReplayStateChangeEvent & change)
{
    ...
    if (!change.is_replaying() &&
        change.replay_mode() == dedupv1::log::EVENT_REPLAY_MODE_DIRTY_START) {
        if (change.success()) {
            CHECK(FinishDirtyLogReplay(),
                "Failed to finish dirty log replay");
        } else{
            WARNING("Dirty log replay failed");
        }
    }
    return true;
}
...
{
    CHECK(RecoverContainerFromPartials(),
        "Failed to recover container from partials");
    return true;
}
...
bool ContainerStorage::FillMergedContainerEventData(
    ...
    slave_address,
    const ContainerStorageAddressData &
    new_container_address)
{
    DCHECK(event_data, "Event data not set");
    DCHECK(IsValidAddressData(
            leader_address), "Invalid leader address data: " <<
        leader_address.ShortDebugString());
    DCHECK(IsValidAddressData(
            slave_address), "Invalid slave address data: " <<
        slave_address.ShortDebugString());
    DCHECK(IsValidAddressData(
            new_container_address), "Invalid new container address data: " <<
        new_container_address.ShortDebugString());
    ...
    for (j = leader_container.secondary_ids().begin();
         j != leader_container.secondary_ids().end();
         j++) {
        event_data->add_first_secondary_id(*j);
    }
    for (j = slave_container.secondary_ids().begin();
         j != slave_container.secondary_ids().end();
         j++) {
...
bool ContainerStorage::TryMergeContainer(uint64_t container_id_1,
    uint64_t container_id_2,
    bool* aborted)
{
    CHECK(state_ == RUNNING || state_ == STARTED,
        "Illegal state to merge container: " << state_);
    ...
    CHECK(
        ...
        "Illegal to merge a container with itself: container " <<
        container_id_1);
    ...
    lookup_result r = this->meta_data_index_->Lookup(&id,
        sizeof(uint64_t),
        &container_address1);
    CHECK(r == LOOKUP_FOUND,
        "Cannot get container address for container " << id);
    CHECK(
        container_address1.has_primary_id() == false,
        "Illegal merge candidate: " <<
    ...
    r = this->meta_data_index_->Lookup(&id,
        sizeof(uint64_t),
        &container_address2);
    CHECK(r == LOOKUP_FOUND,
        "Cannot get container address for container " << id);
    CHECK(
        container_address2.has_primary_id() == false,
        "Illegal merge candidate: " <<
    ...
    cache_lookup_result1 = cache_.CheckCache(
        container1.primary_id(), &cached_container1, true, true, &cache_entry1);
    cache_lookup_result2 = cache_.CheckCache(
        container2.primary_id(), &cached_container2, true, true, &cache_entry2);

    if (cache_lookup_result1 == LOOKUP_ERROR || cache_lookup_result2 ==
        LOOKUP_ERROR) {
        ERROR(
            "Failed to cache read cache for container " <<
            container1.primary_id() <<
            ", " << container2.primary_id());

        if (cache_entry1.is_set()) {
            CHECK(
                cache_entry1.lock()->ReleaseLock(),
                "Failed to release cache lock");
        }
        if (cache_entry2.is_set()) {
            CHECK(
                cache_entry2.lock()->ReleaseLock(),
                "Failed to release cache lock");
        }
    ...
    CHECK(container_lock1.TryAcquireWriteLock(
            &locked), "Cannot acquire container write lock");
    if (!locked) {
    ...
    if (cache_lookup_result1 == LOOKUP_FOUND) {
        cache_remove_result1 = this->cache_.RemoveFromReadCache(
            container1.primary_id(), &cache_entry1);
    }
    if (cache_lookup_result2 == LOOKUP_FOUND) {
        cache_remove_result2 = this->cache_.RemoveFromReadCache(
            container2.primary_id(), &cache_entry2);
    }

    if (!cache_remove_result1) {
        ERROR(
            "Failed to remove container data from cache: container id " <<
            container1.primary_id());
        return false;
    }
    ...
    ScopedInMoveSetMembership scoped_set_membership(&in_move_set_,
        &in_move_set_lock_);
    ...
    if (!scoped_set_membership.Insert(container_id_list)) {
        TRACE(
            "Abort container merge: container in move set: container id " <<
            container_id_1 << ", container id " << container_id_2);
        *aborted = true;
        return true;
    }
    TRACE(
        "Adds containers to in move set: container id " << container_id_1 <<
    ...
    CHECK(this->ReadContainerFromDiskLocked(&container1, container_address1),
        "Failed to read container id " << container_id_1 <<
        ", container " << container1.primary_id());

    // TODO (dmeister) You can also argue that this is more a abort situation than an error
    // situation, but
    // anyways as the gc either does a delete or a merge from only a single thread, there should
    // never be
    ...
    CHECK(this->ReadContainerFromDiskLocked(&container2, container_address2),
    ...
    ContainerStorageAddressData new_container_address;
    enum alloc_result alloc_result = this->allocator_->OnNewContainer(
        new_container,
        false,
        &new_container_address);
    CHECK(alloc_result != ALLOC_ERROR, "Failed to get new container address");
    if (alloc_result == ALLOC_FULL) {
        // this is one of the worst situations to be in. There is no single place left to merge
        // data. So it is not possible to free
        // one container area. But we need COW.
        *aborted = true;
        return true;
    }

    INFO("Merging container: " << container1.DebugString() << "(" <<
        DebugString(container_address1) << ")" <<
        ", " << container2.DebugString() << "(" << DebugString(
            container_address2) << ")" <<
        ", new container " << new_container.DebugString() << "(" <<
        DebugString(new_container_address) << ")");

    CHECK(IsValidAddressData(
            new_container_address), "Invalid address data: " <<
        new_container_address.ShortDebugString());
    ...
    if (container1.HasId(new_container.primary_id())) {
        CHECK(FillMergedContainerEventData(&event_data,
                container1,
                container_address1,
                container2,
                container_address2,
                new_container_address),
            "Cannot fill merged container address");
    ...
    for (set<uint64_t>::const_iterator i = new_container.secondary_ids().begin();
         i != new_container.secondary_ids().end();
         i++) {
    ...
    for (set<uint64_t>::const_iterator i = container1.secondary_ids().begin();
         i != container1.secondary_ids().end();
         i++) {
        all_old_ids.insert(*i);
    }
    all_old_ids.insert(container2.primary_id());
    for (set<uint64_t>::const_iterator i = container2.secondary_ids().begin();
         i != container2.secondary_ids().end();
         i++) {
        all_old_ids.insert(*i);
    }
    ...
    // a client now can read the container at the new or the old position. Both reads must be valid
    // (and the
    ...
    Option<int64_t> commit_result = log_->CommitEvent(EVENT_TYPE_CONTAINER_MERGED,
        &event_data, NO_EC);
    CHECK(commit_result.valid(),
        "Cannot commit merge event data: " << event_data.ShortDebugString());
    CHECK(PostContainerEvent(new_container,
          EVENT_TYPE_CONTAINER_MERGED,
          &event_data,
          commit_result.value()),
        "Failed to handle commit event");
...
bool ContainerStorage::TryDeleteContainer(uint64_t container_id,
    bool* aborted)
{
    CHECK(state_ == STARTED || state_ == RUNNING,
        "Illegal state to delete container: " << state_);
    CHECK(aborted, "Aborted not set");
    CHECK(!start_context_.readonly(), "Container storage is in readonly mode");
    ...
    // we cannot use the normal lookup address method here as that would lead to problems with the
    // locking
    uint64_t id = container.primary_id();
    ContainerStorageAddressData container_address;
    lookup_result r = this->meta_data_index_->Lookup(&id,
    ...
    CHECK(r == LOOKUP_FOUND,
        "Cannot get container address for container " << id);
    CHECK(
        container_address.has_primary_id() == false,
        "Illegal delete candidate: " <<
        "container " << container.DebugString() <<
        ", address " << container_address.ShortDebugString());
    ...
    cache_lookup_result = cache_.CheckCache(
        container.primary_id(), &cached_container, true, true, &cache_entry);
    if (cache_lookup_result == LOOKUP_ERROR) {
    ...
    CHECK(container_lock.TryAcquireWriteLock(
            &locked), "Cannot acquire container write lock");
    if (!locked) {
        if (cache_entry.is_set()) {
            CHECK(
    ...
    if (cache_lookup_result == LOOKUP_FOUND) {
        CHECK(this->cache_.RemoveFromReadCache(
                container.primary_id(),
                &cache_entry),
            "Failed to remove container data from cache: " <<
            container.primary_id());
    }
    ...
    if (!scoped_set_membership.Insert(container_id)) {
        TRACE(
            "Abort container delete: container in move set: container id " <<
            container_id);
        *aborted = true;
        return true;
    }
    ...
    // read container and check if the ids are really the primary ids
    // if not the locks would protect the wrong id.
    CHECK(this->ReadContainerFromDiskLocked(&container, container_address),
    ...
    CHECK(container.primary_id() == container_id, "Container id mismatch: "
        ...
        ", container " << container.DebugString());
    CHECK(
        container.item_count() == 0, "Container is contains items: << " <<
        container.DebugString());

    INFO(
        "Delete container " << container.DebugString() << ", old address " <<
        DebugString(container_address));

    // fill the data for the container merged log event
    ContainerDeletedEventData event_data;
    event_data.set_container_id(container.primary_id());
    for (set<uint64_t>::const_iterator i = container.secondary_ids().begin();
         i != container.secondary_ids().end();
         i++) {
    ...
    Option<int64_t> commit_result = log_->CommitEvent(EVENT_TYPE_CONTAINER_DELETED,
        &event_data, NO_EC);
    CHECK(commit_result.valid(),
...
enum lookup_result ContainerStorage::GetPrimaryId(
    uint64_t container_id,
    uint64_t* primary_id,
    ...
    pair<lookup_result, ContainerStorageAddressData> r =
        this->LookupContainerAddress(container_id,
            primary_container_lock,
            acquire_write_lock);
    CHECK_RETURN(r.first != LOOKUP_ERROR,
        LOOKUP_ERROR,
        "Failed to lookup primary id");
...
void ContainerStorage::ClearData()
{
    ...
    this->state_ = STOPPED;
    if (this->partial_commit_cleanup_thread_.IsJoinable()) {
        this->partial_commit_cleanup_thread_.Join(NULL);
    }
...
uint32_t ContainerStorage::container_size() const
{
